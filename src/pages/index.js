...
export default function Home({ allPostsData }) {
    ...
    const resultadoBusqueda = allPostsData.filter((post) => {
      if (
        post.title
          .toString()
          .toLowerCase()
          .includes(terminoBusqueda.toString().toLowerCase())
      ) {
        return post;
      }
    });
  ...
  return (
    <Layout home>
      ...
      <section
        className={`${utilStyles.headingMd} ${utilStyles.padding1px} ${utilStyles.separation}`}
      >
        <div className={utilStyles.flexContainer}>
          <h2 className={utilStyles.headingLg}>Posts</h2>
          <input
            className={utilStyles.inputBusqueda}
            value={busqueda}
            placeholder="Buscar en este Blog"
